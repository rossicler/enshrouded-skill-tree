import { NodeStatsType } from "./Stats";

export type Node = {
  id: string;
  type: string;
  base?: boolean;
  tier?: "small" | "medium" | "large";
  angle?: number;
  distance?: number;
};

export type NodeTypeMetadata = {
  name: string;
  description: string[];
  hasIcon?: boolean;
  iconOffset?: number;
  hasAsset?: boolean;
  unselectedAsset?: string;
  selectableAsset?: string;
  selectedAsset?: string;
  color: string;
  cost: number;
  stats?: NodeStatsType;
};

export type SkillNodesType = {
  types: { [key: string]: NodeTypeMetadata };
  nodes: { [key: string]: Node };
  edges: { [key: string]: string[] };
};

// ANGLE LINES
export const LinesAngles = [
  15, 45, 75, 105, 135, 165, 195, 225, 255, 285, 315, 345,
];

const SHROUD_TIME_TEXT =
  "<b>Shroud Time</b><br/>Should this time run out, the Shroud will consume you.";
const BLOCK_BREAKER =
  "<b>Block Breaker</b><br/>This special attack fills a blocking enemy's <i>Stun Bar</i> twice as much when hit.";
const OVERPOWER =
  "<b>Overpower</b><br>Attacking blocking enemies or parrying their attacks fills their <i>Stun Bar</i> until they become <b>overpowered</b>.<br><b>Overpowered</b> enemies are open for Merciless Attacks.";
const WET =
  "<b>Wet</b><br/>Reduces Stamina and Stamina Regeneration by <b>30%</b> for <b>15</b> minutes. Warmth reduces the remaining duration.";
const SOAKED =
  "<b>Soaked</b><br/>Reduces Stamina and Stamina Regeneration by <b>30%</b> for <b>30</b> seconds. Also reduces <b>Ice and Shock Resistances</b> by <b>30%</b> but increases <b>Fire Resistances</b> by <b>30%</b>.<br><b>Soaked</b> can be applied to enemies. <br><b>Soaked</b> duration can not be reduced by warmth and <b>Wet</b> debuff starts after it is over.";
const CHARGE =
  "<b>Charge</b><br/>As the Updraft charges before activation, the resulting lift is increased. a full charge grants you an additional boost. The charge is accelerated while inside your base.";
const BLOODRAGE =
  "<b>Blood Rage</b><br/>Increases Melee weapon damage by <b>15%</b> for <b>10</b> seconds.";
canst SKILLSHOT =
  "<b>Skillshot</b><br/>Striking enemy weak points, such as heads or exposed hearts, is considered a Skillshot.";
const SkillNodes: SkillNodesType = {
  types: {
    GIANT_SLAYER_HOOK: {
      name: "GIANT SLAYER HOOK",
      description: [
        "Use your <b>Grappling Hook</b> to pull yourself towards large enemies during combat."
		"<b>Cost:</b> 50 Stamina",
      ],
      hasIcon: true,
      color: "gold",
      cost: 3,
    },
    GROUNDING_HOOK: {
      name: "GROUNDING HOOK",
      description: [
        "Use your <b>Grappling Hook</b> to pull <b>flying</b> enemies towards you during combat."
		"<b>Cost:</b> 50 Stamina",
      ],
      hasIcon: true,
      color: "gold",
      cost: 4,
    },
    SAVIOUR: {
      name: "SAVIOUR",
      description: [
        "Time to revive an ally is reduced by <b>-1</b> seconds.",
		"Default revive time is <b>6</b> seconds. ",
      ],
      color: "gold",
      cost: 1,
	  leveldescription: "<b>-1</b> seconds per level",
	  level: 3,
    },
    BACKSTAB_DAMAGE: {
      name: "BACKSTAB Mastery",
      description: ["<b>Backstab</b> damage is increased by 20%"],
      color: "green",
      cost: 1,
 	  leveldescription: "<b>20%</b> damage per level",
	  level: 3,
   },
    OPPORTUNITY: {
      name: "OPPORTUNITY",
      description: [
        "Increases the damage multiplier of <b>Merciless Attacks</b> by 40%.",
      ],
      color: "gold",
      cost: 1,
 	  leveldescription: "<b>40%</b> damage per level",
	  level: 3,
    },
    MINER: {
      name: "MINER",
      description: [
        "Mining resources has a <b>10%</b> chance to grant one additional resource.",
      ],
      color: "gold",
      cost: 4,
    },
    LUMBERJACK: {
      name: "LUMBERJACK",
      description: [
        "Tool deal <b>10%</b> increased damage against wood.",
		"This includes trees an wooden terrain.",
      ],
      color: "gold",
      cost: 1,
 	  leveldescription: "<b>10%</b> damage per level",
	  level: 3,
    },
    QUALITY_GEAR: {
      name: "QUALITY GEAR",
      description: ["Tools have a <b>20%</b> chance to restore <b>1</b> durability point"],
      color: "gold",
      cost: 2,
    },
    MASON: {
      name: "MASON",
      description: [
        "Tools deal <b>10%</b> increased damage against stone.",
		"This includes stone terrain and gemstone veins.",
      ],
      color: "gold",
      cost: 1,
 	  leveldescription: "<b>10%</b> damage per level",
	  level: 3,
    },
    FISHERMANS_RESOLVE: {
      name: "FISHERMAN'S RESOLVE",
      description: [
        "Increases your Fishing Endurance by <b>5</b>.",
        "Allows for extended battles with hooked fish.",
      ],
      color: "gold",
      cost: 2,
 	  leveldescription: "<b>5</b> Fishing Endurance per level",
	  level: 3,
    },
    POWER_PARRY: {
      name: "POWER PARRY",
      description: [
        "Your Parry Power is increased by <b>10%</b>.",
        "<b>Parry</b><br>A ell-timed block with any Melee weapon or Shield will <b>parry</b> the attack and fill up the enemy's <i>Stun Bar</i>.",
		OVERPOWER
      ],
      color: "gold",
      cost: 1,
  	  leveldescription: "<b>10%</b> Parry Power per level",
	  level: 3,
   },
    ATTR_SPIRIT: {
      name: "SPIRIT",
      description: [
        "Increases your Spirit attribute by 1.",
        "Increases Mana by 20 per Attribute Point.",
      ],
      color: "blue",
      cost: 1,
      stats: { SPIRIT: 1 },
    },
    ATTR_INT: {
      name: "INTELLIGENCE",
      description: [
        "Increases your Intelligence attribute by 1.",
        "Increases Magic damage by 5% per Attribute Point.",
      ],
      color: "blue",
      cost: 1,
      stats: { INT: 1 },
    },
    ATTR_CONS: {
      name: "CONSTITUTION",
      description: [
        "Increases your Constitution attribute by 1.",
        "Increases Health by 50 per Attribute Point.",
      ],
      color: "red",
      cost: 1,
      stats: { CONS: 1 },
    },
    ATTR_STR: {
      name: "STRENGTH",
      description: [
        "Increases your Strength attribute by 1.",
        "Increases Melee damage by 5% per Attribute Point.",
      ],
      color: "red",
      cost: 1,
      stats: { STR: 1 },
    },
    ATTR_ENDURANCE: {
      name: "ENDURANCE",
      description: [
        "Increases your Endurance attribute by 1.",
        "Increases Stamina by 10 per Attribute Point.",
      ],
      color: "green",
      cost: 1,
      stats: { ENDURANCE: 1 },
    },
    ATTR_DEX: {
      name: "DEXTERITY",
      description: [
        "Increases your Dexterity attribute by 1.",
        "Increases Bow and Dagger damage by 5% per Attribute Point.",
      ],
      color: "green",
      cost: 1,
      stats: { DEX: 1 },
    },
    WELL_RESTED: {
      name: "WELL RESTED",
      description: [
        "The base duration for the Rested buff is increased by <b>2</b> minutes.",
        "<b>Rested</b><br>The Rested buff increases your Stamina Maximum and Regeneration significantly.",
		"It requires <b>shelter, sitting or sleeping,</b> as well as <b>warmth</b> from a heat source.",
		"The buff can be refreshed <b>anywhere.</b>"
		"Surrounding comfort items further increase its duration."
      ],
      color: "gold",
      cost: 1,
  	  leveldescription: "<b>2</b> minutes per level",
	  level: 3,
    },
    SNEAK_ATTACK: {
      name: "SNEAK ATTACK",
      description: [
        "Perform a <b>Sneak Attack</b> by sneaking up to an unaware enemy and pressing <b>[E]</b>.",
        "<b>Sneak Attack</b> deals <b>+900%</b> increased damage.",
      ],
      hasIcon: true,
      color: "gold",
      cost: 3,
    },
    MERCILESS_ATTACK: {
      name: "MERCILESS ATTACK",
      description: [
        "Perform a <b>Merciless Attack</b> by pressing <b>[E]</b> to deal <b>+500%</b> damage to an <b>overpowered</b> enemy.",
        OVERPOWER,
      ],
      hasIcon: true,
      color: "gold",
      cost: 2,
    },
    UPDRAFT: {
      name: "UPDRAFT",
      description: [
        "Jumping with [SPACE] while gliding lifts you upwards. Can be used once per flight.",
		CHARGE,
		"<b>Cost:</b> 100 Mana"
      ],
      hasIcon: true,
      color: "green",
      cost: 4,
    },
    BEGONE: {
      name: "BEGONE!",
      description: [
        "A magic-powered punch that pushes back and stuns enemies.",
        "Replaces your unarmed attacks as long as you have the necessary Mana available.",
		"<b>Cost:</b> 30 Mana",
      ],
      hasIcon: true,
      color: "blue",
      cost: 3,
    },
    RADIANT_AURA: {
      name: "RADIANT AURA",
      description: [
        "All <b>Fell</b> enemies within <b>2</b> meters take <b>1 Fire</b> damage per Intelligence per second.",
      ],
      hasIcon: true,
      color: "blue",
      cost: 1,
	  leveldescription: "<b>2</b> meters range and <b>1 Fire</b> damage per Intelligence per level",
	  level: 1,
    },
    WATER_AURA: {
      name: "WATER AURA",
      description: [
        "You emit a healing aura that heals you and all injured allies within <b>5</b> meters.",
		"The healing scales with your intelligence attribute and restores <b>1</b> Health per <b>2</b> points of intelligence.",
      ],
      hasIcon: true,
      color: "blue",
      cost: 1,
	  leveldescription: "<b>5</b> meters per level",
	  level: 3,
    },
    MARTYR: {
      name: "MARTYR",
      description: [
        "When you are killed by an enemy, all allies within 30 meters are healed for <b>50%</b> of their maximum health.",
		"They also receive a <b>Final Blessing</b> buff, increasing their maximum Health by <b>100</b> points for <b>15</b> minutes.",
      ],
      hasIcon: true,
      color: "red",
      cost: 3,
    },
    DIVINE_SURGE: {
      name: "DIVINE SURGE",
      description: [
        "Healing spells can perform Critical Strikes that scale with your Critical Strike damage.",
      ],
      color: "blue",
      cost: 5,
    },
    RIGHTEOUS_FIRE: {
      name: "RIGHTEOUS FIRE",
      description: [
        "Healing any target with a spell triggers a burst of fire that deals <b>20%</b> of the heal amount as <b>Fire</b> damage to nearby <b>Fell</b> enemies.",
      ],
      color: "blue",
      cost: 2,
	  leveldescription: "<b>20% Fire</b> damage per level",
	  level: 3,
    },
    BLINK: {
      name: "BLINK",
      description: [
        "Replaces the Dodge Roll ability with a short-range teleport.",
		"Blink also replaces the sideways Dodge Roll of the Strategig Maneuver skill",
		"<b>Cost:</b> 20 Stamina"
      ],
      hasIcon: true,
      color: "blue",
      cost: 4,
    },
    EVASION_ATTACK: {
      name: "EVASION ATTACK",
      description: [
        "When equipped with a Melee weapon, you can perform an evade attack, which dashes towards the enemy and deals more weapon damage with <b>[LMB]</b>",
		"<b>Cost:</b> 20 Stamina",
        BLOCK_BREAKER,
      ],
      hasIcon: true,
      color: "red",
      cost: 4,
    },
    EARTH_AURA: {
      name: "EARTH AURA",
      description: [
        "You emit a protective aura that increases Physical an Magical Resistances by <b>10%</b> for you and all allies within <b>4</b> meters.",
      ],
      hasIcon: true,
      color: "red",
      cost: 2,
	  leveldescription: "<b>4</b>" meters range per level;
	  level: 3,
    },
    NEMESIS: {
      name: "NEMESIS",
      description: [
        "Increase the attention you draw from enemies by <b>5%</b>.",
      ],
      hasIcon: true,
      color: "red",
      cost: 2,
	  leveldescription: "<b>5%</b> attention drawn per level",
	  level: 3,
    },
    SHOCKWAVE: {
      name: "SHOCKWAVE",
      description: [
        "Trigger a Shockwave when you parry an attack or <b>overpower</b> an enemy.",
        "The Shockwave pushes back nearby enemies. It also fills their <i>Stun Bar</i>, scaling with your Strength attribute.",
		PARRY,
        OVERPOWER,
      ],
      hasIcon: true,
      color: "red",
      cost: 3,
    },
    HEAVY_SPECIALIZATION: {
      name: "HEAVY HITTER",
      description: ["Increases the attack speed of all Two-Handed Melee weapons by <b>10%</b>."],
      hasIcon: true,
      color: "red",
      cost: 5,
    },
    BASH: {
      name: "BASH",
      description: [
        "Parrying enemy attacks bashes them for <b>30 Blunt</b> damage. Bash damage is increased by your Strength attribute.",
		PARRY,
      ],
      hasIcon: true,
      color: "red",
      cost: 2,
	  leveldescription: "<b>10 Blunt</b> damage per level",
	  level: 3,
    },
    WHIRLWIND_CRESCENDO: {
      name: "WHIRLWIND CRESCENDO",
      description: [
        "When equipped with a Two-Handed weapon, trigger a whirl attack at the end of an attack chain.",
        "<b>Cost</b>: 75 Stamina",
      ],
      hasIcon: true,
      color: "red",
      cost: 5,
    },
    CRASH_DOWN_ATTACK: {
      name: "CRASH DOWN ATTACK",
      description: [
        "When equipped with a Melee weapon, you can perform a special attack by holding <b>[LMB]</b> during a jump.",
        "Crash Down deals 50% more weapon damage in a small blast radius and costs Stamina depending on weapon type used.",
        "<b>Cost</b>: 35-60 Stamina",
        BLOCK_BREAKER,
      ],
      hasAsset: true,
      selectedAsset: "crash_down_attack_active.png",
      unselectedAsset: "crash_down_attack.png",
      color: "red",
      cost: 3,
    },
    DOUBLE_JUMP: {
      name: "DOUBLE JUMP",
      description: [
		"Allows jumping a second time while airborne.",
		"<b>Cost:</b> 10 Stamina",
		],
      hasIcon: true,
      iconOffset: 10,
      color: "gold",
      cost: 4,
    },
    DESSERT_STOMACH: {
      name: "DESSERT STOMACH",
      description: ["You gain one additional Food slot"],
      hasIcon: true,
      color: "green",
      cost: 4,
    },
    LAST_MEAL: {
      name: "LAST MEAL",
      description: [
        "Your Food buffs persist through death, but they last half as long.",
      ],
      color: "green",
      cost: 5,
    },
    EAGLE_EYE: {
      name: "EAGLE EYE",
      description: [
        "Greatly increases the zoom while aiming with Bows.",
        "To aim, hold down the [RMB] while a bow is selected in the Action bar. (Alternatively, hold [Q] to aim your equipped bow.)",
      ],
      hasIcon: true,
      color: "green",
      cost: 3,
    },
    MULTI_SHOT: {
      name: "MULTI SHOT",
      description: [
        "Shooting <i>Regular Arrows</i> has a <b>20%</b> chance to spawn a <b>Flurry of <b>Arrows</b> that spreads slightly.",
		"<i>Regular Arrows</i> fired this way will substract from your ammunition.",
        "<b>Flurry of <b>Arrows</b> does not trigger on <i>Special Arrows</i>.",
      ],
      hasIcon: true,
      color: "green",
      cost: 4,
    },
    MULTI_SHOT_SPREAD: {
      name: "MULTI SHOT SPREAD",
      description: [
        "Adds a <b>25%</b> chance to spawn an additional <i>Regular Arrow</i> with your <b>Flurry of <b>Arrows</b> from the <b>Multi Shot</b> skill.",
        "This additional <i>Regular Arrow</i> does not subtract from your ammunition.",
      ],
      color: "green",
      cost: 3,
    },
    MULTI_SHOT_TRIGGER: {
      name: "MULTI SHOT TRIGGER",
      description: [
        "Shooting <i>Special Arrows</i> now triggers a <b>Flurry of <b>Arrows</b> from the <b>Multi Shot</b> skill.",
        "<i>Special Arrows</i> fired this way will subtract from your ammunition.",
      ],
      color: "green",
      cost: 3,
    },
    BEE_STING: {
      name: "BEE STING",
      description: [
        "Firing or aiming your Bow while airborne slowes down your fall briefly. Every shot lifts you slightly, granting more airtime.",
        "<b>Cost:</b> 10 stamina per second",
      ],
      hasIcon: true,
      color: "green",
      cost: 3,
    },
    SHELL_SHOCK: {
      name: "SHELL SHOCK",
      description: [
        "Infuse your <b>Ranged Explosives</b> with Mana. They now stun enemies for 2 seconds.",
        "<b>Cost:</b> 8 Mana per stunned enemy",
      ],
      hasIcon: true,
      color: "green",
      cost: 3,
    },
    COUNTERSTRIKE: {
      name: "COUNTERSTRIKE",
      description: [
        "After receiving damage, there is a <b>8%</b> chance to reflect <b>20%</b> of the damage back to the attacker as <b>Fire</b> damage.",
        "This magical attack can trigger other skills.",
      ],
      color: "blue",
      cost: 1,
	  leveldescription: "<b>8%</b> chance and <b>20%</b> reflected damage per level",
	  level: 3,
    },
    TERROR: {
      name: "TERROR",
      description: [
        "Critical Strikes with spells have a <b>15%</b> chance to stun the target for <b>2</b> seconds.",
      ],
      color: "blue",
      cost: 2,
	  leveldescription: "<b>15%</b> chance per level",
	  level: 3,
    },
    SHOCK_RESISTANCE: {
      name: "SHOCK RESISTANCE",
      description: [
        "Increases your <b>Shock Resistance</b> by <b>4%</b> which reduces the amount of <b>Shock</b> damage received.",
      ],
      color: "blue",
      cost: 2,
	  leveldescription: "<b>4%</b> Resistance per level",
	  level: 3,
    },
    QUICK_CHARGE: {
      name: "QUICK CHARGE",
      description: [
        "Cast Time of spells is reduced by <b>-15%</b>.",
      ],
      color: "blue",
      cost: 1,
	  leveldescription: "<b>-15%</b> Cast Time per level",
	  level: 3,
    },
    THIS_IS_THE_WAY: {
      name: "MAGE APPRENTICE",
      description: [
        "Magic weapon damage is increased by <b>5%</b>.",
      ],
      color: "blue",
      cost: 1,
	  leveldescription: "<b>5%</b> damage per level",
	  level: 3,
    },
    ARSONIST: {
      name: "ARSONIST",
      description: ["All <b>Fire</b> damage is increased by <b>10%</b>."],
      color: "blue",
      cost: 2,
	  leveldescription:"<b>10%</b> damage per level",
	  level: 3,
    },
    PYROMANIAC: {NaN
    },
    SUN_AURA: {
      name: "RADIANT AURA",
      description: [
        "All <b>Fell</b> enemies within <b>2</b> meters take <b>1 Fire</b> damage per Intelligence per second.",
      ],
      color: "blue",
      cost: 1,
	  leveldescription: "<b>2</b> meters range and <b>1 Fire</b> damage per Intelligence per level",
	  level: 3,
    },
    FIRE_RESISTANCE: {
      name: "FIRE RESISTANCE",
      description: [
        "Increases your <b>Fire Resistance</b> by <b>4%</b>, which reduces the amount of <b>Fire</b> damage received.",
      ],
      color: "blue",
      cost: 2,
	  leveldescription: "<b>4%</b> Resistance per level",
	  level: 3,
    },
    THUNDER: {
      name: "THUNDER",
      description: ["All <b>Shock</b> damage is increased by <b>10%</b>."],
      color: "blue",
      cost: 2,
	  leveldescription:"<b>10%</b> damage per level",
	  level: 3,
    },
    LIGHTNING: {NaN
    },
    ICEMAN: {
      name: "ICEMAN",
      description: ["All <b>Ice</b> damage is increased by <b>10%</b>."],
      color: "blue",
      cost: 2,
	  leveldescription: "<b>10%</b> damage per level",
	  level: 3,
    },
    SUBZERO: {NaN
    },
    FROST: {
      name: "FROST",
      description: [
        "When receiving Melee damage, the attacker will be slowed down for 3 seconds.",
      ],
      color: "blue",
      cost: 2,
	  leveldescription:"<b>3s</b> per level",
    },
    ICE_RESISTANCE: {
      name: "ICE RESISTANCE",
      description: [
        "Increases your <b>Ice Resistance</b> by <b>4%</b> which reduces the amount of <b>Ice</b> damage received.",
      ],
      color: "blue",
      cost: 2,
	  leveldescription:"<b>4%</b> Resistance per level",
	  level: 3,
    },
    WIZARD: {
      name: "WIZARD",
      description: [
        "Magic weapon Critical Strike chance is increased by <b>2%</b>.",
      ],
      color: "blue",
      cost: 2,
	  leveldescription:"<b>2%</b> Critical Strike chance per level",
	  level: 3,
    },
    DARK_ARTS: {
      name: "DARK ARTS",
      description: ["All <b>Shroud damage</b> is increased by <b>10%</b>."],
      color: "blue",
      cost: 2,
	  leveldescription: "<b>10%</b> damage per level",
	  level: 3,
    },
    ABYSS: {NaN
    },
    CHAIN_HIT: {
      name: "CHAIN HIT",
      description: [
        "Critical Strikes with Magic weapon will automatically hit a second enemy within <b>15</b> meters for <b>5 Shock</b> damage per intelligence.",
      ],
      color: "blue",
      cost: 3,
    },
    MASS_DESTRUCTION: {
      name: "MASS DESTRUCTION",
      description: [
        "Critical Strikes with a Magic weapons deal <b>2 Shock</b> damage per intelligence to all enemies whithin <b>5</b> meters of the target.",
      ],
      color: "blue",
      cost: 2,
	  leveldescription: "<b>5</b> meters range per level",
	  level: 3,
    },
    NECROMANCER: {
      name: "NECROMANCER",
      description: [
        "When killing an enemy with a Magic weapon, you have a <b>5%</b> chance to summon a friendly <b>Skull Companion</b>.",
      ],
      color: "blue",
      cost: 1,
	  leveldescription: "<b>5%</b> chance per level",
	  level: 3,
    },
    HEALER: {
      name: "HEALER",
      description: [
        "Health gain from healing spells and skills is increased by <b>15%</b>.",
      ],
      color: "blue",
      cost: 2,
	  leveldescription: "<b>15%</b> healing per level",
	  level: 3,
    },
    HEALER_II: {NaN
    },
    HEALING_REVIVE: {
      name: "HEALING REVIVE",
      description: [
        "Revive players with <b>8%</b> increased Health.",
		"default Health of revived players is <b>10%</b>.",
      ],
      color: "blue",
      cost: 1,
	  leveldescription: "<b>8%</b> Health per level",
    },
    SHROUD_FILTER: {
      name: "SHROUD FILTER",
      description: [
        "Dealing damage with a Magic weapon has a <b>5%</b> chance to trigger a small flame burst that restores <b>9</b> seconds of <b>Time in the Shroud</b> to you and your allies within <b>20</b> meters.",
        SHROUD_TIME_TEXT,
      ],
      color: "blue",
      cost: 1,
	  leveldescription: "<b>5%</b> chance per level",
	  level: 3,
    },
    WATERS_OF_LIFE: {
      name: "WATERS OF LIFE",
      description: [
        "Increases <b>Water Aura</b> healing to <b>2</b> Health per <b>2</b> points of Intelligence.",
      ],
      color: "blue",
      cost: 2,
    },
    SHROUD_RESISTANCE: {
      name: "SHROUD RESISTANCE",
      description: [
        "Increases your <b>Shroud Resistance</b> by <b>4%</b> which reduces the amount of <b>Shroud</b> damage received.",
      ],
      color: "blue",
      cost: 2,
	  leveldescription: "<b>4%</b> Resistance per level",
	  level: 3,
    },
    SHROUD_MISTERY: {
      name: "SHARED MISERY",
      description: [
        "When you receive <b>Shroud</b> damage, all players within <b>5</b>-meters receive <b>15%</b> increased <b>Shroud Resistance</b> for <b>30</b> seconds.",
      ],
      color: "blue",
      cost: 2,
	  leveldescription: "<b>5</b> meters per level",
	  level: 3,
    },
    EMERGENCY_BLINK: {
      name: "EMERGENCY BLINK",
      description: [
        "You can blink while being stunned. This will break the stunned state.",
      ],
      color: "blue",
      cost: 2,
    },
    BLINK_ATTACK: {
      name: "BLINK ATTACK",
      description: [
        "Blinking into an enemy triggers an explosion which deals <b>Fire</b> damage in a small radius.",
        "Damage and Mana cost scale with the Intelligence attribute.",
      ],
      color: "blue",
      cost: 2,
    },
    ARCANE_DEFLECTION: {
      name: "ARCANE DEFLECTION",
      description: [
	  "Successfully parrying an enemy's attack restores <b>10</b> Mana."
	  PARRY,
	  ],
      color: "blue",
      cost: 1,
	  leveldescription: "<b>10</b> Mana per level",
	  level: 3,
    },
    UNITY: {
      name: "UNITY",
      description: [
        "Damaging enemies with Wands has a <b>10%</b> chance to restore 2% of your maximum Mana.",
      ],
      hasIcon: true,
      color: "blue",
      cost: 1,
	  leveldescription: "<b>10%</b> chance and <b>2%</b> of maximum Mana per level",
	  level: 3,
    },
    WAND_MASTER: {
      name: "ARCANE PROLIFERATION",
      description: [
        "Wand attacks have a <b>30%</b> chance to spawn double the amount of projectiles. Extra projectilea spawned deal <b>50%</b> less damage.",
      ],
      color: "blue",
      cost: 3,
    },
    ETERNAL_SPARK: {
      name: "ETERNAL SPARK",
      description: [
        "Dealing damage with Wand projectiles has a <b>10%</b> chance to restore 1 durability to that weapon.",
      ],
      color: "blue",
      cost: 1,
	  leveldescription: "<b>10%</b> chance per level",
	  level: 3,
    },
    STING: {
      name: "STING",
      description: ["Repeated Wand damage is increased by <b>20%</b>."],
      color: "blue",
      cost: 3,
    },
    BATTLE_HEAL: {
      name: "BATTLE HEAL",
      description: [
        "Critical Strikes with Melee weapon heal you for <b>1%</b> of your maximum Health.",
      ],
      color: "red",
      cost: 2,
	  leveldescription: "<b>1%</b> of maximum Health per level",
	  level: 3,
    },
    BLOODLETTING: {
      name: "BLOODLETTING",
      description: [
        "Critical Strikes with spells have a <b>15%</b> chance to spawn 2 Health, Mana, and/or Stamina Orbs.",
        "Gathering an Orb restores <b>10%</b> of the respective resource.",
      ],
      color: "blue",
      cost: 2,
	  leveldescription: "<b>15%</b> chance per level",
	  level: 3,
    },
    LIFE_BURST: {
      name: "LIFE BURST",
      description: [
        "Defeating an enemy with a Magic weapon restores Health equal to <b>3</b> times your Intelligence to all players within <b>15</b> meters of the target..",
      ],
      color: "blue",
      cost: 4,
    },
    BLOOD_MAGIC: {
      name: "BLOOD MAGIC",
      description: [
        "When your Mana drops below <b>20%</b>, you restore up to <b>35%</b> of your maximum Mana at the cost of <b>1</b> Health per Mana restored."
		"This effect stops at 1 Health.",
        "<b>Cooldown:</b> 2 minutes",
      ],
      color: "blue",
      cost: 4,
    },
    ABSORB: {
      name: "ABSORB",
      description: [
        "When you receive <b>Magic</b> damage, you have a <b>4%</b> chance to restore 1 Mana per Health point lost.",
      ],
      color: "red",
      cost: 1,
	  leveldescription: "<b>4%</b> chance per level",
	  level: 3,
    },
    SNAP: {
      name: "SNAP",
      description: ["Triggering a <b>Merciless Attack</b> restores <b>10%</b> Mana."],
      color: "red",
      cost: 3,
    },
    SOUL_LEECH: {
      name: "SOUL LEECH",
      description: [
        "When killing an enemy with a Melee weapon, all players within <b>5</b> meters of the target restore <b>15</b> Mana.",
      ],
      color: "red",
      cost: 2,
	  leveldescription: "<b>5</b> meters range and <b>15</b> Mana per level",
	  level: 3,
    },
    SHINY_PLATES: {
      name: "SHINY PLATES",
      description: ["Equipped armor grants <b>4%</b> increased Magical Armor"],
      color: "red",
      cost: 1,
	  leveldescription; "<b>4%</b> increased Magical Armor per level",
	  level: 3,
    },
    HEAVY_PLATES: {
      name: "HEAVY PLATES",
      description: [
        "Equipped armor grants <b>4%</b> increased Physical Armor",
      ],
      color: "red",
      cost: 1,
 	  leveldescription; "<b>4%</b> increased Physical Armor per level",
	  level: 3,
   },
    WARDEN: {
      name: "WARDEN",
      description: [
        "While there are three or more enemies within <b>20</b> meters, you receive <b>10%</b> increased <b>Magical Resistance</b>.",
      ],
      color: "red",
      cost: 3,
    },
    TOWER: {
      name: "TOWER",
      description: [
        "While there are three or more enemies within <b>20</b> meters, you receive <b>10%</b> increased <b>Physical Resistance<b/>",
      ],
      color: "red",
      cost: 3,
    },
    ARCH_NEMESIS: {NaN
    },
    PURIFICATION: {
      name: "PURIFICATION",
      description: [
        "Defeating a Shroud infested enemy with a Melee weapon restore <b>3</b> seconds of <b>Time in the Shrous</b>.",
        SHROUD_TIME_TEXT,
      ],
      color: "red",
      cost: 1,
	  leveldescription: "<b>3</b> seconds per level",
	  level: 3,
    },
    WARRIOR_PATH: {
      name: "WARRIOR'S PATH",
      description: [
        "One-Handed Melee weapon damage is increased by <b>5%</b>.",
      ],
      color: "red",
      cost: 1,
	  leveldescription: "<b>5%</b> damage per level",
	  level: 3
    },
    THRUST: {
      name: "THRUST",
      description: [
        "Mlee <b>Piercing</b> damage is increased by <b>10%</b>.",
      ],
      color: "red",
      cost: 2,
	  leveldescription: "<b>10%</b> damage per level",
	  level: 3,
    },
    PIERCE: {NaN
    },
    BRUTE: {
      name: "BRUTE",
      description: [
        "Melee <b>Blunt</b> damage is increased by <b>10%</b>.",
      ],
      color: "red",
      cost: 2,
	  leveldescription: "<b>10%</b> damage per level",
	  level: 3,
    },
    HAMMER_TIME: {NaN
    },
    SLASHER: {
      name: "SLASHER",
      description: [
        "Melee <b>Cutting</b> damage is increased by <b>10%</b>.",
      ],
      color: "red",
      cost: 2,
	  leveldescription: "<b>10%</b> damage per level",
	  level: 3,
    },
    BUTCHER: {NaN
    },
    VETERAN: {
      name: "VETERAN",
      description: [
        "Melee weapon Critical Strike chance is increased by <b>2%</b>.",
      ],
      color: "red",
      cost: 2,
	  leveldescription: "<b>2%</b> Critical Strike chance per level",
	  level: 3,
    },
    TITAN_EDGE: {
      name: "TITAN EDGE",
      description: ["Two-Handed Greatsword damage is increased by <b>5%</b>."],
      color: "red",
      cost: 2,
	  leveldescription: "<b>5%</b> damage per level",
	  level: 3,
    },
    SWIFT_BLADES: {
      name: "SWIFT BLADES",
      description: [
        "Increase the attack speed of all One-Handed Melee weapons by <b>15%</b>.",
      ],
      hasIcon: true,
      color: "red",
      cost: 5,
    },
    FEAST: {NaN
    },
    HEAVY_HANDED: {NaN
    },
    STEADFAST: {
      name: "STEADFAST",
      description: [
        "Defeating an enemy with a melee weapon restores <b>1</b> durability to that weapon.",
      ],
      color: "red",
      cost: 2,
	  leveldescription: "<b>1</b> Durability per level",
    },
    UPWARDS_SLASH_ATTACK: {
      name: "UPWARDS SLASH ATTACK",
      description: [
        "When equipped with a Melee weapon, launch a powerful upward strike during the ascent of your jump.",
        "Use this skill to rapidly close the distance on <b>flying</b> enemies.",
        "<b>Cost</b>: 25 Stamina",
      ],
      hasIcon: true,
      color: "red",
      cost: 3,
    },
    BREACH: {
      name: "BREACH",
      description: [
        "<b>Overpowered</b> enemies receive <b>+100%</b> increased Melee physical damage for <b>3</b> seconds.",
        OVERPOWER,
      ],
      color: "red",
      cost: 3,
    },
    RELENTLESS: {
      name: "RELENTLESS",
      description: [
        "Critical Strikes with Two-Handed weapons increase your Critical Strike chance by <b>8%</b> for the next hit.",
      ],
      color: "red",
      cost: 5,
    },
    BARBARIAN: {
      name: "BARBARIAN",
      description: [
        "Gain one point of Strength for every two levels of the Flame.",
      ],
      color: "red",
      cost: 5,
    },
    BLOOD_RAGE: {
      name: "BLOOD RAGE",
      description: [
        "When an enemy within <b>5</b> meters is killed by a Melee weapon, you fall into <b>Blood Rage</b>",
		BLOODRAGE,
      ],
	  leveldescription: "<b>5</b> meters per level",
	  level; 3.
      color: "red",
      cost: 2,
    },
    VIGOROUS_DEFLECTION: {
      name: "VIGOROUS DEFLECTION",
      description: [
	  "Succesfully parrying an enemy's attack restore <b>15</b> Stamina.",
	  PARRY,
	  ],
	  leveldescription: "<b>15</b> Stamina per level",
	  level: 3,
      color: "red",
      cost: 1,
    },
    BLOOD_WARRIOR: {
      name: "BLOOD WARRIOR",
      description: [
        "Defeating an enemy with a <b>Sneak Attack</b> or <b>Merciless Attack</b> spawns a Health Orb.",
        "Gathering an Orb restores <b>10%</b> of the respective resource.",
      ],
      color: "red",
      cost: 3,
    },
    WET_DOG: {
      name: "WET DOG",
      description: [
        "Weakens the effect of the <b>Wet</b> and <b>Soaked</b> debuffs.",
        "Stamina and Stamina Regeneration is reduced by <b>20%</b> instead of <b>30%</b>.",
        WET,
        SOAKED,
      ],
      color: "red",
      cost: 3,
    },
    SOAKED_DOG: {
      name: "SOAKED DOG",
      description: [
        "Weakens the effect of the <b>Wet</b> and <b>Soaked</b> debuffs.",
        "Stamina and Stamina Regeneration are reduced by <b>10%</b> instead of <b>20%</b>.",
        WET,
        SOAKED,
      ],
      color: "red",
      cost: 3,
    },
    SPLASH_DASH: {
      name: "SPLASH DASH",
      description: [
        "Allows you to perform an evasive dash while swimming or diving.",
        "<b>Cost</b>: 40 Stamina",
      ],
      hasIcon: true,
      color: "red",
      cost: 5,
    },
    FINESSE: {
      name: "FINESSE",
      description: [
        "Dealing damage with One-Handed Melee weapons and daggers has a <b>10%</b> chance to restore <b>1</b> durability to that weapon.",
      ],
      color: "red",
      cost: 1,
	  leveldescription: "<b>10%</b> chance per level",
	  level: 3,
    },
    JUMP_ATTACK_II: {
      name: "CRASH DOWN: FORCE",
      description: [
        "When attacking from a double jump, <b>Crash Down</b> deals <b>20%</b> more weapon damage.",
      ],
      color: "red",
      cost: 3,
    },
    BACKSTAB_MASTERY: {
      name: "BACKSTAB MASTERY",
      description: ["<b>Backstab</b> damage is increased by <b>20%</b>."],
      color: "green",
      cost: 1,
	  leveldescription: "<b>20%</b> per level",
	  level: 3,
    },
    RUNNER: {
      name: "RUNNER",
      description: [
        "Sprint Speed is increased by <b>5%</b> and Stamina cost while sprinting is decreased by <b>5%</b>.",
      ],
      color: "green",
      cost: 1,
	  leveldescription: "<b>5%</b> Speed and <b>5%</b> cost reduction per level",
    },
    WANDERLUST: {
      name: "WANDERLUST",
      description: [
        "Stamina cost for <b>sprinting on dirt roads</b> is reduced from <b>90% to 80%</b>. Stamina cost for <b>sprinting on stone roads</b> is reduced from <b>75% to 50%</b>.",
      ],
      color: "green",
      cost: 3,
    },
    GOOD_METABOLISM: {
      name: "GOOD METABOLISM",
      description: [
        "Health, Mana, and Stamina Orbs restore <b>30%</b> instead of <b>10%</b> of their respective resource.",
        "Health and Mana Potions restore <b>20%</b> more resources.",
      ],
      color: "green",
      cost: 3,
    },
    SWIFTSHOT_SUSTENANCE: {
      name: "SWIFTSHOT SUSTENANCE",
      description: [
        "Defeating an enemy with a Bow has a <b>12%</b> chance to spawn a Stamina Orb."
		"Gathering an Orb restores <b>10%</b> of the respective resource.",
      ],
      color: "green",
      cost: 1,
	  leveldescription: "<b>12%</b> chance per level",
	  level: 3,
    },
    SWEET_TOOTH: {
      name: "SWEET TOOTH",
      description: ["Stamina Regeneration of sweets is increased by <b>50%</b>."],
      color: "green",
      cost: 3,
    },
    ARACHNOID: {
      name: "ARACHNOID",
      description: ["Stamina cost while climbing is reduced by <b>20%</b>."],
      color: "green",
      cost: 1,
	  leveldescription: "<b>20%</b> cost reduction per level",
    },
    REBOUND: {
      name: "REBOUND",
      description: ["Base Stamina Regeneration is increased <b>10%</b>."],
      color: "green",
      cost: 2,
	  leveldescription: "<b>10%</b> Stamina Regeneration per level",
	  level: 3,
    },
    REBOUND_II: {NaN
    },
    INNER_FIRES: {
      name: "INNER FIRES",
      description: [
        "<i>Time in the Shroud</i> is increased by <b>2 min</b>, allowing you to explore for longer.",
        SHROUD_TIME_TEXT,
      ],
      color: "green",
      cost: 2,
	  leveldescription: "<b>2 min</b> per level",
	  level: 3,
    },
    RELENTLESS_FLAME: {NaN
    },
    SNAKE_EATER: {
      name: "SNAKE EATER",
      description: [
        "Increases your <b>Poison Resistance</b> by <b>4%</b>, which reduces the amount of <b>Poison</b> damage received.",
      ],
      color: "green",
      cost: 2,
	  leveldescription: "<b>4%</b> Resistance per level",
	  level: 3,
    },
    MITHRIDATIST: {
      name: "MITHRIDATIST",
      description: [
        "Gain a <b>25%</b> chance to avoid being poisoned.",
      ],
      color: "green",
      cost: 2,
    },
    DAGGER_MASTER: {NaN
    },
    SLICE_AND_DICE: {
      name: "SLICE AND DICE",
      description: [
        "After a Critical Strike with a Dagger, the base damage of your next Bow attack within 20 seconds is increased by <b>50%</b>.",
      ],
      color: "green",
      cost: 3,
    },
    VUKAH_LANGUAGE: {
      name: "VUKAH LANGUAGE",
      description: [
        "<b>Vukah</b> within <b>50</b> meters will no longer attack you unless provoked.",
      ],
      color: "green",
      cost: 3,
    },
    CALM_SPIRIT: {
      name: "CALM SPIRIT",
      description: [
        "<b>Wild Animals</b> within <b>50</b> meters will be no longer attack you unless provoked.",
		"Does not affect animals corrupted by the Shroud.",
      ],
      color: "green",
      cost: 3,
    },
    BEAST_MASTER: {
      name: "BEAST MASTER",
      description: [
        "When you are targeted by an attack, <b>Wild Animals</b> within <b>50</b> meters will attack the enemy.",
      ],
      color: "green",
      cost: 4,
    },
    VUKAH_CULTURE: {
      name: "VUKAH CULTURE",
      description: [
        "When you are targeted by an attack, <b>Vukah</b> within <b>50</b> meters will attack the enemy.",
      ],
      color: "green",
      cost: 4,
    },
    ENDURANCE_OF_THE_FLAME: {
      name: "ENDURANCE OF THE FLAME",
      description: [
        "Gain one point of Endurance for every two levels of the Flame.",
      ],
      color: "green",
      cost: 5,
    },
    MARKSMAN: {
      name: "MARKSMAN",
      description: ["Damage with Bows is increased by <b>5%</b>"],
      color: "green",
      cost: 1,
	  leveldescription: "<b>5%</b> damage per level",
    },
    SHARPSHOOTER: {NaN
    },
    COUNTER_BATTERY: {
      name: "COUNTER BATTERY",
      description: ["Damage against <b>ranged</b> enemies is increased by <b>8%</b>."],
      color: "green",
      cost: 1,
	  leveldescription: "<b>8%</b> damage per level",
	  level: 3,
    },
    EAGLES_BANE: {
      name: "EAGLES BANE",
      description: ["Damage against <b>flying</b> enemies is increased by <b>10%</b>."],
      color: "green",
      cost: 1,
	  leveldescription: "<b>10%</b> damage per level",
      level; 3,
	},
    SKILL_SHOT: {
      name: "SKILL SHOT",
      description: [
		"<b>Skillshot</b> damage is increased by <b>20%</b>.",
		SKILLSHOT,
	  ],
      color: "green",
      cost: 1,
	  leveldescription: "<b>20%</b> damage per level",
	  level: 3,
    },
    RANGER: {
      name: "RANGER",
      description: [
        "Increases your Dexterity and Endurace attributes by <b>1</b>.",
		"Base Stamina Regeneration is increased by <b>5</b>,",
      ],
      color: "green",
      cost: 2,
      stats: {
        DEX: 1,
        ENDURANCE: 1,
      },
	  leveldescription: [
	    "<b>1</b> Dexterity and Endurance per level.",
	    "<b>5</b> Stamina Regeneration per lavel.",
	  ]
	  level: 3,
    },
    SILENT_STRIDE: {
      name: "SILENT STRIDE",
      description: ["Increases your movement speed while <b>sneaking</b> by <b>20%</b>."],
      color: "green",
      cost: 1,
	  leveldescription: "<b>20%</b> movement speed per level",
	  level: 3,
    },
    KICK: {
      name: "KICK",
      description: [
        "When equipped with a Melee weapon you can perform a Kick by attacking while blocking.",
        "Kick deals little damage but hits the target with massive force and pushes them back, filling their <i>Stun Bar</i>.",
        "<i>Stun Bar</i> increase scales with Dexterity. Stun time depends on the enemy's size.",
		OVERPOWER,
      ],
      color: "green",
      hasIcon: true,
      cost: 3,
    },
    AIRBORNE: {
      name: "AIRBORNE",
      description: ["Gliders consume <b>12%</b> less Stamina"],
      color: "green",
      cost: 2,
	  leveldescription: "<b>12%</b> cost reduction per level",
	  level: 3,
    },
    SNIPER: {
      name: "SNIPER",
      description: [
        "Ranged weapon Critical Strike chance is increased by <b>2%</b>.",
      ],
      color: "green",
      cost: 2,
	  leveldescription: "<b>2%</b> Critical Strike chance per level",
	  level: 3,
    },
    VITALITY_SURGE: {
      name: "VITALITY SURGE",
      description: [
        "Critical Strikea with Ranged weapons restore <b>3</b> Stamina.",
      ],
      color: "green",
      cost: 3,
	  leveldescription: "<b>3</b> Stamina per level",
	  level: 3,
    },
    BLESSED_ARROWS: {
      name: "BLESSED ARROWS",
      description: [
        "Critical Strikes with Bowsrestore <b>8</b> mana.",
      ],
      color: "green",
      cost: 1,
	  leveldescription: "<b>8</b> mana per level",
	  level: 3,
    },
    BOUNTY_BONANZA: {
      name: "BOUNTY BONANZA",
      description: [
        "After defeating a <b>Fell</b> enemy with a <b>Skillshot</b>, your group gains an additional <b>+5</b> Experience Points.",
		SKILLSHOT,
      ],
      color: "green",
      cost: 2,
    },
    RICOCHETS: {
      name: "RICOCHETS",
      description: [
        "For every target you hit with an <b>Explosive Arrow</b>, its damage is increased by <b>1%</b>.",
      ],
      color: "green",
      cost: 4,
    },
    GRACEFUL_STRIDE: {
      name: "GRACEFUL STRIDE",
      description: [
        "Gain one point of Dexterity for every two levels of the Flame.",
      ],
      color: "green",
      cost: 5,
    },
    CHAIN_REACTION: {
      name: "CHAIN REACTION",
      description: [
        "Every enemy hit with an <b>Explosive Arrow</b> has a <b>20%</b> chance to trigger a secondary explosion for <b>50%</b> damage in a small radius.",
      ],
      color: "green",
      cost: 5,
    },
    ARCANE_CONCENTRATION: {
      name: "ARCANE CONCENTRATION",
      description: [
        "Gain one point of Spirit for every two levels of the Flame.",
      ],
      color: "blue",
      cost: 5,
    },
    EXALTED: {
      name: "EXALTED",
      description: [
        "Gain one point of Intelligence for every two levels of the Flame.",
      ],
      color: "blue",
      cost: 5,
    },
    THICK_SKIN: {
      name: "THICK SKIN",
      description: [
        "Gain one point of Constitution for every two levels of the Flame.",
      ],
      color: "red",
      cost: 5,
    },
    LIFE_ESSENCES: {
      name: "LIFE ESSENCES",
      description: [
        "Increase your maximum Health by <b>2</b> times your Intelligence attribute.",
      ],
      color: "blue",
      hasIcon: true,
	  cost: 2
	  leveldescription: "<b>2</b> times per level",
	  level: 3,
    },
    POISONED_BLADES: {
      name: "VENOMOUS BLADES",
      description: [
        "Increases the chance to poison enemies to <b>25%</b> when using Daggers with the <b>Venomous Blades</b> perk.",
      ],
      color: "green",
      cost: 3,
    },
    POISON_MASTERY: {
      name: "VENOM MASTERY",
      description: [
        "Increases the damage dealt with the <b>Venomous Blades</b> Dagger perk by <b>100%</b>.",
      ],
      color: "green",
      cost: 4,
    },
    QUICK_REFLEX_BLOCK: {
      name: "QUICK REFLEX BLOCK",
      description: [
        "Briefly enhances your block value when blocking with Daggers, ensuring a near-miss parry reduces stamina minimally.",
      ],
      color: "green",
      cost: 3,
    },
    EXPOSE_WEAKNESS: {
      name: "EXPOSE WEAKNESS",
      description: [
        "Critical Strikes with Daggers increase damage dealt to the enemy by <b>25%</b> for <b>5</b> seconds.",
      ],
      color: "green",
      cost: 4,
    },
  },

  nodes: {
    "1": {
      id: "1",
      type: "WELL_RESTED",
      base: true,
      tier: "medium",
      distance: -120,
      angle: 255,
    },
    "2": {
      id: "2",
      type: "SNEAK_ATTACK",
      base: true,
      tier: "large",
      distance: -120,
      angle: 225,
    },
    "3": {
      id: "3",
      type: "BACKSTAB_DAMAGE",
      tier: "medium",
      distance: -190,
      angle: 225,
    },
    "181": {
      id: "181",
      type: "OPPORTUNITY",
      tier: "medium",
      distance: -250,
      angle: 225,
    },
    "4": {
      id: "4",
      type: "LUMBERJACK",
      base: true,
      tier: "medium",
      distance: -120,
      angle: 165,
    },
    "5": {
      id: "5",
      type: "QUALITY_GEAR",
      tier: "medium",
      distance: -120,
      angle: 120,
    },
    "6": {
      id: "6",
      type: "MINER",
      tier: "medium",
      distance: -190,
      angle: 120
    },
    "7": {
      id: "7",
      type: "MASON",
      base: true,
      tier: "medium",
      distance: -120,
      angle: 75,
    },
    "220": {
      id: "220",
      type: "FISHERMANS_RESOLVE",
      base: true,
      tier: "medium",
      distance: -120,
      angle: 45,
    },
    "8": {
      id: "8",
      type: "MERCILESS_ATTACK",
      base: true,
      tier: "large",
      distance: -120,
      angle: 15,
    },
    "9": {
      id: "9",
      type: "POWER_PARRY",
      tier: "medium",
      distance: -190,
      angle: 15,
    },
    "10": {
      id: "10",
      type: "SAVIOUR",
      base: true,
      tier: "medium",
      distance: -120,
      angle: 315,
    },
    "11": {
      id: "11",
      type: "ATTR_SPIRIT",
      base: true,
      tier: "small",
      angle: 255,
    },
    "12": {
      id: "12",
      type: "COUNTERSTRIKE",
      tier: "medium",
      distance: 70,
      angle: 255,
    },
    "13": {
      id: "13",
      type: "ATTR_INT",
      tier: "small",
      distance: 170,
      angle: 250,
    },
    "14": {
      id: "14",
      type: "ATTR_SPIRIT",
      tier: "small",
      distance: 170,
      angle: 260,
    },
    "15": {
      id: "15",
      type: "BEGONE",
      tier: "large",
      distance: 240,
      angle: 255,
    },
    "17": {
      id: "17",
      type: "ATTR_SPIRIT",
      tier: "small",
      distance: 310,
      angle: 250,
    },
    "16": {
      id: "16",
      type: "ATTR_INT",
      tier: "small",
      distance: 310,
      angle: 260,
    },
    "68": {
      id: "68",
      type: "BLOOD_MAGIC",
      tier: "medium",
      distance: 380,
      angle: 250,
    },
    "64": {
      id: "64",
      type: "BLOODLETTING",
      tier: "medium",
      distance: 380,
      angle: 255,
    },
    "18": {
      id: "18",
      type: "TERROR",
      tier: "medium",
      distance: 380,
      angle: 260,
    },
    "217": {
      id: "217",
      type: "SHOCK_RESISTANCE",
      tier: "medium",
      distance: 450,
      angle: 263,
    },
    "218": {
      id: "218",
      type: "SHOCK_RESISTANCE",
      tier: "medium",
      distance: 520,
      angle: 263,
    },
    "219": {
      id: "219",
      type: "SHOCK_RESISTANCE",
      tier: "medium",
      distance: 590,
      angle: 263,
    },
    "19": {
      id: "19",
      type: "ARCANE_CONCENTRATION",
      tier: "small",
      distance: 450,
      angle: 255,
    },
    "20": {
      id: "20",
      type: "QUICK_CHARGE",
      tier: "medium",
      distance: 120,
      angle: 270,
    },
    "21": {
      id: "21",
      type: "ATTR_SPIRIT",
      base: true,
      angle: 285
    },
    "22": {
      id: "22",
      type: "THIS_IS_THE_WAY",
      tier: "medium",
      distance: 70,
      angle: 285,
    },
    "23": {
      id: "23",
      type: "NECROMANCER",
      tier: "medium",
      distance: 120,
      angle: 300,
    },
    "24": {
      id: "24",
      type: "ATTR_SPIRIT",
      distance: 140,
      angle: 285
    },
    "25": {
      id: "25",
      type: "ARSONIST",
      tier: "medium",
      distance: 210,
      angle: 277,
    },
    "26": {
      id: "26",
      type: "THUNDER",
      tier: "medium",
      distance: 210,
      angle: 285,
    },
    "27": {
      id: "27",
      type: "ICEMAN",
      tier: "medium",
      distance: 210,
      angle: 294,
    },
    "211": {
      id: "211",
      type: "ICE_RESISTANCE",
      tier: "medium",
      distance: 250,
      angle: 300,
    },
    "28": {
      id: "28",
      type: "PYROMANIAC",
      tier: "medium",
      distance: 280,
      angle: 277,
    },
    "29": {
      id: "29",
      type: "RADIANT_AURA",
      tier: "large",
      distance: 350,
      angle: 273,
    },
    "30": {
      id: "30",
      type: "SUN_AURA",
      tier: "medium",
      distance: 420,
      angle: 273,
    },
    "214": {
      id: "214",
      type: "FIRE_RESISTANCE",
      tier: "medium",
      distance: 490,
      angle: 271,
    },
    "215": {
      id: "215",
      type: "FIRE_RESISTANCE",
      tier: "medium",
      distance: 560,
      angle: 271,
    },
    "216": {
      id: "216",
      type: "FIRE_RESISTANCE",
      tier: "medium",
      distance: 630,
      angle: 271,
    },
    "31": {
      id: "31",
      type: "ATTR_SPIRIT",
      distance: 490,
      angle: 276
    },
    "32": {
      id: "32",
      type: "LIGHTNING",
      tier: "medium",
      distance: 280,
      angle: 285,
    },
    "33": {
      id: "33",
      type: "SUBZERO",
      tier: "medium",
      distance: 280,
      angle: 294,
    },
    "212": {
      id: "212",
      type: "ICE_RESISTANCE",
      tier: "medium",
      distance: 320,
      angle: 300,
    },
    "34": {
      id: "34",
      type: "FROST",
      tier: "medium",
      distance: 350,
      angle: 294,
    },
    "213": {
      id: "213",
      type: "ICE_RESISTANCE",
      tier: "medium",
      distance: 390,
      angle: 300,
    },
    "35": {
      id: "35",
      type: "ATTR_INT",
      distance: 350,
      angle: 285
    },
    "36": {
      id: "36",
      type: "WIZARD",
      tier: "medium",
      distance: 420,
      angle: 285,
    },
    "37": {
      id: "37",
      type: "DARK_ARTS",
      tier: "medium",
      distance: 490,
      angle: 282,
    },
    "38": {
      id: "38",
      type: "CHAIN_HIT",
      tier: "medium",
      distance: 490,
      angle: 288,
    },
    "39": {
      id: "39",
      type: "ABYSS",
      tier: "medium",
      distance: 560,
      angle: 282,
    },
    "40": {
      id: "40",
      type: "MASS_DESTRUCTION",
      tier: "medium",
      distance: 560,
      angle: 288,
    },
    "41": {
      id: "41",
      type: "ATTR_INT",
      distance: 630,
      angle: 285
    },
    "42": {
      id: "42",
      type: "ATTR_INT",
      base: true,
      angle: 315
    },
    "43": {
      id: "43",
      type: "HEALER",
      tier: "medium",
      distance: 70,
      angle: 315,
    },
    "44": {
      id: "44",
      type: "HEALER_II",
      tier: "medium",
      distance: 140,
      angle: 315,
    },
    "45": {
      id: "45",
      type: "ATTR_INT",
      distance: 210,
      angle: 315
    },
    "46": {
      id: "46",
      type: "WATER_AURA",
      tier: "large",
      distance: 280,
      angle: 320,
    },
    "47": {
      id: "47",
      type: "HEALING_REVIVE",
      tier: "medium",
      distance: 280,
      angle: 310,
    },
    "48": {
      id: "48",
      type: "SHROUD_FILTER",
      tier: "medium",
      distance: 330,
      angle: 325,
    },
    "49": {
      id: "49",
      type: "WATERS_OF_LIFE",
      tier: "medium",
      distance: 330,
      angle: 315,
    },
    "205": {
      id: "205",
      type: "SHROUD_RESISTANCE",
      tier: "medium",
      distance: 390,
      angle: 330,
    },
    "206": {
      id: "206",
      type: "SHROUD_RESISTANCE",
      tier: "medium",
      distance: 460,
      angle: 330,
    },
    "207": {
      id: "207",
      type: "SHROUD_RESISTANCE",
      tier: "medium",
      distance: 530,
      angle: 330,
    },
    "208": {
      id: "208",
      type: "SHROUD_MISTERY",
      tier: "medium",
      distance: 600,
      angle: 330,
    },
    "50": {
      id: "50",
      type: "ATTR_SPIRIT",
      distance: 340,
      angle: 306
    },
    "51": {
      id: "51",
      type: "ATTR_INT",
      distance: 410,
      angle: 312
    },
    "52": {
      id: "52",
      type: "ATTR_SPIRIT",
      distance: 410,
      angle: 318
    },
    "53": {
      id: "53",
      type: "MARTYR",
      tier: "large",
      distance: 480,
      angle: 315,
    },
    "209": {
      id: "209",
      type: "DIVINE_SURGE",
      tier: "medium",
      distance: 530,
      angle: 320,
    },
    "210": {
      id: "210",
      type: "RIGHTEOUS_FIRE",
      tier: "medium",
      distance: 560,
      angle: 315,
    },
    "55": {
      id: "55",
      type: "BLINK",
      tier: "large",
      distance: 120,
      angle: 330
    },
    "56": {
      id: "56",
      type: "EMERGENCY_BLINK",
      tier: "medium",
      distance: 180,
      angle: 327,
    },
    "57": {
      id: "57",
      type: "BLINK_ATTACK",
      tier: "medium",
      distance: 180,
      angle: 333,
    },
    "62": {
      id: "62",
      type: "ATTR_SPIRIT",
      distance: 240,
      angle: 330
    },
    "58": {
      id: "58",
      type: "ATTR_INT",
      base: true,
      angle: 345
    },
    "59": {
      id: "59",
      type: "ARCANE_DEFLECTION",
      tier: "medium",
      distance: 70,
      angle: 345,
    },
    "60": {
      id: "60",
      type: "EVASION_ATTACK",
      tier: "large",
      distance: 120,
      angle: 360,
    },
    "61": {
      id: "61",
      type: "BATTLE_HEAL",
      tier: "medium",
      distance: 190,
      angle: 360,
    },
    "69": {
      id: "69",
      type: "ATTR_INT",
      distance: 140,
      angle: 345
    },
    "70": {
      id: "70",
      type: "UNITY",
      tier: "large",
      distance: 210,
      angle: 345
    },
    "72": {
      id: "72",
      type: "STING",
      tier: "medium",
      distance: 270,
      angle: 350,
    },
    "66": {
      id: "66",
      type: "LIFE_BURST",
      tier: "medium",
      distance: 330,
      angle: 354,
    },
    "71": {
      id: "71",
      type: "WAND_MASTER",
      tier: "medium",
      distance: 270,
      angle: 340,
    },
    "180": {
      id: "180",
      type: "ETERNAL_SPARK",
      tier: "medium",
      distance: 330,
      angle: 336,
    },
    "73": {
      id: "73",
      type: "ATTR_SPIRIT",
      distance: 350,
      angle: 348
    },
    "74": {
      id: "74",
      type: "ATTR_INT",
      distance: 350,
      angle: 342
    },
    "188": {
      id: "188",
      type: "LIFE_ESSENCES",
      tier: "large",
      distance: 420,
      angle: 345,
    },
    "54": {
      id: "54",
      type: "EXALTED",
      distance: 490,
      angle: 345
    },
    "75": {
      id: "75",
      type: "ATTR_CONS",
      base: true,
      angle: 15
    },
    "76": {
      id: "76",
      type: "SHINY_PLATES",
      tier: "medium",
      distance: 70,
      angle: 15,
    },
    "77": {
      id: "77",
      type: "HEAVY_PLATES",
      tier: "medium",
      distance: 140,
      angle: 15,
    },
    "78": {
      id: "78",
      type: "ATTR_CONS",
      tier: "small",
      distance: 210,
      angle: 15,
    },
    "79": {
      id: "79",
      type: "TOWER",
      tier: "medium",
      distance: 280,
      angle: 20
    },
    "80": {
      id: "80",
      type: "WARDEN",
      tier: "medium",
      distance: 280,
      angle: 10,
    },
    "63": {
      id: "63",
      type: "ABSORB",
      tier: "medium",
      distance: 350,
      angle: 5,
    },
    "65": {
      id: "65",
      type: "SNAP",
      tier: "medium",
      distance: 420,
      angle: 5
    },
    "67": {
      id: "67",
      type: "SOUL_LEECH",
      tier: "medium",
      distance: 490,
      angle: 5,
    },
    "81": {
      id: "81",
      type: "ATTR_STR",
      distance: 350,
      angle: 25
    },
    "82": {
      id: "82",
      type: "ATTR_STR",
      distance: 350,
      angle: 18
    },
    "83": {
      id: "83",
      type: "ATTR_CONS",
      distance: 350,
      angle: 12
    },
    "84": {
      id: "84",
      type: "NEMESIS",
      tier: "large",
      distance: 420,
      angle: 25,
    },
    "85": {
      id: "85",
      type: "ARCH_NEMESIS",
      tier: "medium",
      distance: 490,
      angle: 25,
    },
    "86": {
      id: "86",
      type: "EARTH_AURA",
      tier: "large",
      distance: 480,
      angle: 15,
    },
    "87": {
      id: "87",
      type: "THICK_SKIN",
      distance: 550,
      angle: 15
    },
    "88": {
      id: "88",
      type: "ATTR_CONS",
      base: true,
      angle: 45
    },
    "89": {
      id: "89",
      type: "WARRIOR_PATH",
      tier: "medium",
      distance: 70,
      angle: 45,
    },
    "90": {
      id: "90",
      type: "PURIFICATION",
      tier: "medium",
      distance: 120,
      angle: 30,
    },
    "91": {
      id: "91",
      type: "ATTR_STR",
      distance: 140,
      angle: 45
    },
    "92": {
      id: "92",
      type: "SLASHER",
      tier: "medium",
      distance: 210,
      angle: 55,
    },
    "93": {
      id: "93",
      type: "BRUTE",
      tier: "medium",
      distance: 210,
      angle: 45
    },
    "94": {
      id: "94",
      type: "THRUST",
      tier: "medium",
      distance: 210,
      angle: 35,
    },
    "95": {
      id: "95",
      type: "BUTCHER",
      tier: "medium",
      distance: 280,
      angle: 55,
    },
    "96": {
      id: "96",
      type: "HAMMER_TIME",
      tier: "medium",
      distance: 280,
      angle: 45,
    },
    "97": {
      id: "97",
      type: "PIERCE",
      tier: "medium",
      distance: 280,
      angle: 35,
    },
    "98": {
      id: "98",
      type: "TITAN_EDGE",
      tier: "medium",
      distance: 350,
      angle: 50,
    },
    "204": {
      id: "204",
      type: "VETERAN",
      tier: "medium",
      distance: 350,
      angle: 40,
    },
    "99": {
      id: "99",
      type: "ATTR_CONS",
      distance: 420,
      angle: 50
    },
    "100": {
      id: "100",
      type: "ATTR_STR",
      distance: 420,
      angle: 40
    },
    "101": {
      id: "101",
      type: "SWIFT_BLADES",
      tier: "large",
      distance: 490,
      angle: 45,
    },
    "102": {
      id: "102",
      type: "ATTR_CONS",
      distance: 560,
      angle: 45
    },
    "103": {
      id: "103",
      type: "ATTR_STR",
      base: true,
      angle: 75
    },
    "104": {
      id: "104",
      type: "HEAVY_HANDED",
      tier: "medium",
      distance: 70,
      angle: 75,
    },
    "183": {
      id: "183",
      type: "STEADFAST",
      tier: "medium",
      distance: 120,
      angle: 90,
    },
    "202": {
      id: "202",
      type: "UPWARDS_SLASH_ATTACK",
      tier: "large",
      distance: 190,
      angle: 90,
    },
    "105": {
      id: "105",
      type: "ATTR_STR",
      distance: 140,
      angle: 75
    },
    "106": {
      id: "106",
      type: "FEAST",
      tier: "medium",
      distance: 120,
      angle: 60,
    },
    "107": {
      id: "107",
      type: "ATTR_CONS",
      distance: 210,
      angle: 75
    },
    "108": {
      id: "108",
      type: "RELENTLESS",
      tier: "medium",
      distance: 280,
      angle: 80,
    },
    "109": {
      id: "109",
      type: "BREACH",
      tier: "medium",
      distance: 280,
      angle: 70,
    },
    "110": {
      id: "110",
      type: "SHOCKWAVE",
      tier: "large",
      distance: 350,
      angle: 60,
    },
    "111": {
      id: "111",
      type: "ATTR_CONS",
      distance: 380,
      angle: 75
    },
    "112": {
      id: "112",
      type: "HEAVY_SPECIALIZATION",
      tier: "large",
      distance: 450,
      angle: 75,
    },
    "113": {
      id: "113",
      type: "BARBARIAN",
      distance: 520,
      angle: 75
    },
    "114": {
      id: "114",
      type: "BLOOD_RAGE",
      tier: "medium",
      distance: 590,
      angle: 75,
    },
    "115": {
      id: "115",
      type: "ATTR_STR",
      base: true,
      angle: 105
    },
    "116": {
      id: "116",
      type: "CRASH_DOWN_ATTACK",
      tier: "large",
      distance: 70,
      angle: 105,
    },
    "117": {
      id: "117",
      type: "DOUBLE_JUMP",
      tier: "large",
      distance: 120,
      angle: 120,
    },
    "118": {
      id: "118",
      type: "JUMP_ATTACK_II",
      tier: "medium",
      distance: 190,
      angle: 120,
    },
    "119": {
      id: "119",
      type: "ATTR_STR",
      distance: 190,
      angle: 105
    },
    "120": {
      id: "120",
      type: "BLOOD_WARRIOR",
      tier: "medium",
      distance: 340,
      angle: 105,
    },
    "199": {
      id: "199",
      type: "WET_DOG",
      tier: "medium",
      distance: 470,
      angle: 105,
    },
    "200": {
      id: "200",
      type: "SOAKED_DOG",
      tier: "medium",
      distance: 490,
      angle: 100,
    },
    "201": {
      id: "201",
      type: "SPLASH_DASH",
      tier: "large",
      distance: 550,
      angle: 105,
    },
    "182": {
      id: "182",
      type: "FINESSE",
      tier: "medium",
      distance: 260,
      angle: 110,
    },
    "178": {
      id: "178",
      type: "BACKSTAB_MASTERY",
      tier: "medium",
      distance: 310,
      angle: 120,
    },
    "121": {
      id: "121",
      type: "VIGOROUS_DEFLECTION",
      tier: "medium",
      distance: 260,
      angle: 100,
    },
    "122": {
      id: "122",
      type: "ATTR_CONS",
      distance: 620,
      angle: 105
    },
    "123": {
      id: "123",
      type: "ATTR_CONS",
      distance: 400,
      angle: 110
    },
    "124": {
      id: "124",
      type: "ATTR_STR",
      distance: 400,
      angle: 100
    },
    "125": {
      id: "125",
      type: "BASH",
      tier: "large",
      distance: 310,
      angle: 90
    },
    "126": {
      id: "126",
      type: "ATTR_STR",
      distance: 380,
      angle: 90
    },
    "203": {
      id: "203",
      type: "WHIRLWIND_CRESCENDO",
      tier: "large",
      distance: 480,
      angle: 90,
    },
    "127": {
      id: "127",
      type: "ATTR_ENDURANCE",
      base: true,
      angle: 135
    },
    "128": {
      id: "128",
      type: "RUNNER",
      tier: "medium",
      distance: 70,
      angle: 135,
    },
    "129": {
      id: "129",
      type: "INNER_FIRES",
      tier: "medium",
      distance: 120,
      angle: 150,
    },
    "179": {
      id: "179",
      type: "RELENTLESS_FLAME",
      tier: "medium",
      distance: 190,
      angle: 150,
    },
    "130": {
      id: "130",
      type: "ATTR_ENDURANCE",
      distance: 140,
      angle: 135
    },
    "131": {
      id: "131",
      type: "WANDERLUST",
      tier: "medium",
      distance: 210,
      angle: 135,
    },
    "132": {
      id: "132",
      type: "GOOD_METABOLISM",
      tier: "medium",
      distance: 280,
      angle: 140,
    },
    "133": {
      id: "133",
      type: "SWIFTSHOT_SUSTENANCE",
      tier: "medium",
      distance: 280,
      angle: 130,
    },
    "134": {
      id: "134",
      type: "REBOUND",
      distance: 300,
      angle: 135
    },
    "197": {
      id: "197",
      type: "REBOUND",
      distance: 340,
      angle: 135
    },
    "198": {
      id: "198",
      type: "REBOUND_II",
      distance: 380,
      angle: 135
    },
    "135": {
      id: "135",
      type: "SWEET_TOOTH",
      tier: "medium",
      distance: 370,
      angle: 140,
    },
    "136": {
      id: "136",
      type: "ARACHNOID",
      tier: "medium",
      distance: 370,
      angle: 130,
    },
    "137": {
      id: "137",
      type: "ATTR_DEX",
      distance: 440,
      angle: 135
    },
    "138": {
      id: "138",
      type: "DESSERT_STOMACH",
      tier: "large",
      distance: 510,
      angle: 135,
    },
    "196": {
      id: "196",
      type: "LAST_MEAL",
      tier: "medium",
      distance: 510,
      angle: 130,
    },
    "139": {
      id: "139",
      type: "ATTR_ENDURANCE",
      distance: 580,
      angle: 135
    },
    "140": {
      id: "140",
      type: "ATTR_ENDURANCE",
      base: true,
      angle: 165
    },
    "141": {
      id: "141",
      type: "SNAKE_EATER",
      tier: "medium",
      distance: 70,
      angle: 165,
    },
    "142": {
      id: "142",
      type: "EAGLE_EYE",
      tier: "large",
      distance: 120,
      angle: 180,
    },
    "143": {
      id: "143",
      type: "MITHRIDATIST",
      tier: "medium",
      distance: 140,
      angle: 165,
    },
    "144": {
      id: "144",
      type: "ATTR_ENDURANCE",
      distance: 210,
      angle: 165
    },
    "184": {
      id: "184",
      type: "DAGGER_MASTER",
      tier: "medium",
      distance: 280,
      angle: 160,
    },
    "189": {
      id: "189",
      type: "POISONED_BLADES",
      tier: "medium",
      distance: 350,
      angle: 153,
    },
    "190": {
      id: "190",
      type: "POISON_MASTERY",
      tier: "medium",
      distance: 420,
      angle: 155,
    },
    "185": {
      id: "185",
      type: "SLICE_AND_DICE",
      tier: "medium",
      distance: 280,
      angle: 170,
    },
    "191": {
      id: "191",
      type: "QUICK_REFLEX_BLOCK",
      tier: "medium",
      distance: 350,
      angle: 177,
    },
    "195": {
      id: "195",
      type: "EXPOSE_WEAKNESS",
      tier: "medium",
      distance: 420,
      angle: 175,
    },
    "145": {
      id: "145",
      type: "VUKAH_LANGUAGE",
      tier: "medium",
      distance: 420,
      angle: 170,
    },
    "146": {
      id: "146",
      type: "CALM_SPIRIT",
      tier: "medium",
      distance: 420,
      angle: 160,
    },
    "147": {
      id: "147",
      type: "ATTR_DEX",
      distance: 350,
      angle: 168
    },
    "148": {
      id: "148",
      type: "ATTR_ENDURANCE",
      distance: 350,
      angle: 162
    },
    "149": {
      id: "149",
      type: "VUKAH_CULTURE",
      tier: "medium",
      distance: 510,
      angle: 170,
    },
    "150": {
      id: "150",
      type: "BEAST_MASTER",
      tier: "medium",
      distance: 510,
      angle: 160,
    },
    "151": {
      id: "151",
      type: "ATTR_DEX",
      distance: 465,
      angle: 165
    },
    "152": {
      id: "152",
      type: "ENDURANCE_OF_THE_FLAME",
      distance: 580,
      angle: 165,
    },
    "153": {
      id: "153",
      type: "ATTR_DEX",
      base: true,
      angle: 195
    },
    "154": {
      id: "154",
      type: "MARKSMAN",
      tier: "medium",
      distance: 70,
      angle: 195,
    },
    "155": {
      id: "155",
      type: "SILENT_STRIDE",
      tier: "medium",
      distance: 120,
      angle: 210,
    },
    "194": {
      id: "194",
      type: "KICK",
      tier: "large",
      distance: 200,
      angle: 210,
    },
    "156": {
      id: "156",
      type: "SHARPSHOOTER",
      tier: "medium",
      distance: 200,
      angle: 200,
    },
    "157": {
      id: "157",
      type: "COUNTER_BATTERY",
      tier: "medium",
      distance: 200,
      angle: 190,
    },
    "158": {
      id: "158",
      type: "EAGLES_BANE",
      tier: "medium",
      distance: 320,
      angle: 200,
    },
    "159": {
      id: "159",
      type: "SKILL_SHOT",
      tier: "medium",
      distance: 320,
      angle: 190,
    },
    "160": {
      id: "160",
      type: "BEE_STING",
      tier: "large",
      distance: 400,
      angle: 205,
    },
    "161": {
      id: "161",
      type: "RANGER",
      tier: "medium",
      distance: 420,
      angle: 195,
    },
    "162": {
      id: "162",
      type: "MULTI_SHOT",
      tier: "large",
      distance: 400,
      angle: 185,
    },
    "186": {
      id: "186",
      type: "MULTI_SHOT_SPREAD",
      tier: "medium",
      distance: 470,
      angle: 183,
    },
    "187": {
      id: "187",
      type: "MULTI_SHOT_TRIGGER",
      tier: "medium",
      distance: 530,
      angle: 185,
    },
    "163": {
      id: "163",
      type: "ATTR_DEX",
      base: true,
      angle: 225
    },
    "164": {
      id: "164",
      type: "AIRBORNE",
      tier: "medium",
      distance: 70,
      angle: 225,
    },
    "165": {
      id: "165",
      type: "UPDRAFT",
      tier: "large",
      distance: 120,
      angle: 240,
    },
    "166": {
      id: "166",
      type: "ATTR_ENDURANCE",
      distance: 140,
      angle: 225
    },
    "167": {
      id: "167",
      type: "SNIPER",
      tier: "medium",
      distance: 210,
      angle: 225,
    },
    "168": {
      id: "168",
      type: "VITALITY_SURGE",
      tier: "medium",
      distance: 280,
      angle: 230,
    },
    "169": {
      id: "169",
      type: "BLESSED_ARROWS",
      tier: "medium",
      distance: 280,
      angle: 220,
    },
    "170": {
      id: "170",
      type: "ATTR_DEX",
      distance: 350,
      angle: 235
    },
    "171": {
      id: "171",
      type: "SHELL_SHOCK",
      tier: "large",
      distance: 420,
      angle: 235,
    },
    "172": {
      id: "172",
      type: "BOUNTY_BONANZA",
      tier: "medium",
      distance: 350,
      angle: 225,
    },
    "173": {
      id: "173",
      type: "ATTR_DEX",
      distance: 350,
      angle: 215
    },
    "174": {
      id: "174",
      type: "RICOCHETS",
      tier: "medium",
      distance: 420,
      angle: 225,
    },
    "175": {
      id: "175",
      type: "GRACEFUL_STRIDE",
      distance: 490,
      angle: 225
    },
    "176": {
      id: "176",
      type: "ATTR_ENDURANCE",
      distance: 490,
      angle: 220
    },
    "177": {
      id: "177",
      type: "CHAIN_REACTION",
      tier: "medium",
      distance: 560,
      angle: 225,
    },
    "192": {
      id: "192",
      type: "GIANT_SLAYER_HOOK",
      base: true,
      tier: "large",
      distance: -100,
      angle: 285,
    },
    "193": {
      id: "193",
      type: "GROUNDING_HOOK",
      tier: "large",
      distance: -180,
      angle: 295,
    },
  },
  edges: {
    "1": [],
    "2": ["3"],
    "3": ["2", "181"],
    "181": ["3", "9"],
    "4": ["5", "6"],
    "5": ["4", "7"],
    "6": ["4", "7"],
    "7": ["5", "6"],
    "8": ["9"],
    "9": ["8", "181"],
    "10": [],
    "11": ["12"],
    "12": ["11", "13", "14", "20", "165"],
    "13": ["12", "15"],
    "14": ["12", "15"],
    "15": ["13", "14", "16", "17"],
    "16": ["15", "18", "64"],
    "17": ["15", "68", "64"],
    "18": ["16", "19", "217"],
    "19": ["18", "68"],
    "20": ["12", "22"],
    "21": ["22"],
    "22": ["20", "24", "23", "21"],
    "23": ["22", "43"],
    "24": ["22", "25", "26", "27"],
    "25": ["24", "28"],
    "26": ["24", "32"],
    "27": ["24", "33", "211"],
    "28": ["25", "29", "35"],
    "29": ["28", "30"],
    "30": ["29", "31", "214"],
    "31": ["30"],
    "32": ["26", "35"],
    "33": ["27", "35", "34", "212"],
    "34": ["33", "213"],
    "35": ["28", "32", "33", "36"],
    "36": ["35", "37", "38"],
    "37": ["36", "39"],
    "38": ["36", "40"],
    "39": ["37"],
    "40": ["38", "41"],
    "41": ["40"],
    "42": ["43"],
    "43": ["42", "23", "44", "55"],
    "44": ["43", "45"],
    "45": ["44", "47", "46"],
    "46": ["45", "48", "49", "52"],
    "47": ["45", "50", "51"],
    "48": ["46", "205"],
    "49": ["46"],
    "50": ["47"],
    "51": ["47", "53"],
    "52": ["46", "53"],
    "53": ["51", "52", "209", "210"],
    "54": ["188"],
    "55": ["43", "56", "57", "59"],
    "56": ["55", "62"],
    "57": ["55", "62"],
    "58": ["59"],
    "59": ["58", "55", "69", "60"],
    "60": ["59", "76", "61"],
    "61": ["60"],
    "62": ["56", "57"],
    "63": ["65"],
    "64": ["16", "17"],
    "65": ["63", "67"],
    "66": ["72"],
    "67": ["65"],
    "68": ["17", "19"],
    "69": ["59", "70"],
    "70": ["69", "72", "73", "71", "74"],
    "71": ["70", "180"],
    "72": ["70", "66"],
    "73": ["70", "188"],
    "74": ["70", "188"],
    "75": ["76"],
    "76": ["75", "60", "77", "90"],
    "77": ["76", "78"],
    "78": ["77", "80", "79"],
    "79": ["78", "81", "82"],
    "80": ["78", "83", "63"],
    "81": ["79", "84"],
    "82": ["79", "86"],
    "83": ["80", "86"],
    "84": ["81", "85"],
    "85": ["84"],
    "86": ["82", "83", "87"],
    "87": ["86"],
    "88": ["89"],
    "89": ["88", "90", "91", "106"],
    "90": ["89", "76"],
    "91": ["89", "92", "93", "94"],
    "92": ["91", "95"],
    "93": ["91", "96"],
    "94": ["91", "97"],
    "95": ["92", "98"],
    "96": ["93", "98", "204"],
    "97": ["94", "204"],
    "98": ["95", "96", "99", "101"],
    "99": ["98"],
    "100": ["204"],
    "101": ["98", "102", "204"],
    "102": ["101"],
    "103": ["104"],
    "104": ["103", "106", "105", "183"],
    "183": ["104", "116", "202"],
    "105": ["104", "107"],
    "106": ["104", "89"],
    "107": ["105", "108", "109"],
    "108": ["107", "111", "125"],
    "109": ["107", "111", "110"],
    "110": ["109"],
    "111": ["108", "109", "112"],
    "112": ["111", "113"],
    "113": ["112", "114"],
    "114": ["113"],
    "115": ["116"],
    "116": ["115", "117", "119", "183"],
    "117": ["116", "118", "128"],
    "118": ["117"],
    "119": ["116", "182", "121"],
    "120": ["182", "121", "123", "124"],
    "182": ["119", "178", "120"],
    "121": ["119", "125", "120"],
    "122": ["201"],
    "123": ["199", "120"],
    "124": ["199", "120"],
    "125": ["121", "108", "126"],
    "126": ["125", "203"],
    "127": ["128"],
    "128": ["127", "117", "130", "129"],
    "129": ["128", "141", "179"],
    "130": ["128", "131"],
    "131": ["130", "132", "133"],
    "132": ["131", "135", "134"],
    "133": ["131", "136", "178", "134"],
    "134": ["132", "197", "133"],
    "135": ["132", "137"],
    "136": ["133", "137"],
    "137": ["135", "136", "138"],
    "138": ["137", "139", "196"],
    "139": ["138"],
    "140": ["141"],
    "141": ["140", "129", "143", "142"],
    "142": ["141", "154"],
    "143": ["141", "144"],
    "144": ["143", "185", "184"],
    "184": ["144", "148", "189"],
    "185": ["144", "147", "191"],
    "145": ["147", "151", "149"],
    "146": ["148", "151", "150"],
    "147": ["145", "185"],
    "148": ["184", "146"],
    "149": ["145", "152"],
    "150": ["146", "152"],
    "151": ["145", "146"],
    "152": ["149", "150"],
    "153": ["154"],
    "154": ["153", "142", "155", "156", "157"],
    "155": ["154", "164", "194"],
    "156": ["154", "159"],
    "157": ["154", "158"],
    "158": ["157", "160", "161"],
    "159": ["156", "161", "162"],
    "160": ["158"],
    "161": ["158", "159"],
    "162": ["159", "186"],
    "186": ["162", "187"],
    "187": ["186"],
    "163": ["164"],
    "164": ["163", "165", "166", "155"],
    "165": ["164", "12"],
    "166": ["164", "167"],
    "167": ["166", "168", "169"],
    "168": ["167", "170", "172"],
    "169": ["167", "172", "173"],
    "170": ["168", "171"],
    "171": ["170"],
    "172": ["168", "169", "174"],
    "173": ["169"],
    "174": ["175", "176", "172"],
    "175": ["174", "177"],
    "176": ["174"],
    "177": ["175"],
    "178": ["182", "133"],
    "179": ["129"],
    "180": ["71", "205"],
    "188": ["54", "73", "74"],
    "189": ["184", "190"],
    "190": ["189"],
    "191": ["185", "195"],
    "192": ["193"],
    "193": ["192"],
    "194": ["155"],
    "195": ["191"],
    "196": ["138"],
    "197": ["134", "198"],
    "198": ["197"],
    "199": ["123", "124", "200", "201"],
    "200": ["199"],
    "201": ["199", "122"],
    "202": ["183"],
    "203": ["126"],
    "204": ["96", "97", "100", "101"],
    "205": ["48", "206", "180"],
    "206": ["205", "207"],
    "207": ["206", "208"],
    "208": ["207"],
    "209": ["53"],
    "210": ["53"],
    "211": ["27"],
    "212": ["33"],
    "213": ["34"],
    "214": ["30", "215"],
    "215": ["214", "216"],
    "216": ["215"],
    "217": ["18", "218"],
    "218": ["217", "219"],
    "219": ["218"],
    "220": [],
  },
};

export default SkillNodes;
